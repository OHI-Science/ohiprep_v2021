---
title: 'OHI 2020: Commercial Fishing Pressure Layers '
...
**Reference**: 
 ...
 2. Watson, R. (2017) A database of global marine commercial, small-scale, illegal and unreported fisheries catch 1950-2017. Nature Scientific Data 4 (170039).

**Downloaded**: December, 11, 2019 from [IMAS portal]() - see mazu readme for v5 link
...
library(readxl)
setwd(here::here("globalprep","prs_fish","v2020"))

source('http://ohi-science.org/ohiprep_v2020/workflow/R/common.R')

rawFolder <- file.path(dir_M, "git-annex/globalprep/_raw_data/IMAS_GlobalFisheriesLandings/d2020")
rastFolder <- paste0(dir_M,"/git-annex/globalprep/prs_fish/v2020/int/")
...
* Catch (1950 - 2017) - reported, iuu, and discard catch data for each cell location and unique identifier for industrial and non-industrial vessels
...
Explore the new watson data: 
 - Here I am testing to see if the columns in the new watson data match the file columns from previous years. 
```{r, eval=FALSE}

df <- readRDS(file.path(dir_M,"/git-annex/globalprep/prs_fish/v2019/int/annual_catch/CatchNInd_1950.rds"))

# indexNInd <- read.csv(file.path(dir_M,"git-annex/globalprep/_raw_data/IMAS_GlobalFisheriesLandings/d2019/IndexNInd.csv"))

data <- readRDS(file.path(dir_M, "git-annex/globalprep/_raw_data/IMAS_GlobalFisheriesLandings/d2020/Catch2015_2019.rds"))

test <- data %>%
  dplyr::filter(Cell == 39098, CNumber == 124, ReportedIND ==0 , IUUIND == 0, DiscardsIND == 0, FGearCode == 11, IYear == 1950, Taxonkey == 602674) %>%
  mutate(total = ReportedNIND + IUUNIND + DiscardsNIND) ## 0.00449

test_ly <- df %>%
  filter(Cell == 39098, FleetGearName == "Gillnet", TaxonName == "Coregonus nasus") %>%
  mutate(total = Reported + IUU + Discards) # 0.00492

# these match pretty well, makes me more optimistic. Let's try another. 

test2 <- data %>%
  filter(Cell == 39098, CNumber == 124, ReportedIND ==0 , IUUIND == 0, DiscardsIND == 0, FGearCode == 11, IYear == 1950, Taxonkey == 400183) %>%
  mutate(total = ReportedNIND + IUUNIND + DiscardsNIND) ## 0.02308

test_ly2 <- df %>%
  filter(Cell == 39098, FleetGearName == "Gillnet", TaxonName == "Gadidae") %>%
  mutate(total = Reported + IUU + Discards) # 0.04094

# not as well... but the methods and data collection did change from v4 to v5. 


## lets check to see if there are similar numbers of observations for just 1950 Nind
test3 <- data %>%
  filter(ReportedIND ==0 , IUUIND == 0, DiscardsIND == 0, IYear == 1950) # 263,465

test_ly3 <- df %>%
  filter(Year == 1950) #437,817

test_ly4 <- indexNInd %>%
  filter(IYear == 1950)


## I believe this has convinced me that the v5 data provided by Watson is already prepped. I.e. there were no index files to join with the raw data, because they already completed that for us. We will need to prep the data to match the columns in our annual_catch.rds files, so we can complete the same analysis as previous years. 

## When we data check, if there aren't crazy differences, it is probably ok.
```
...
1. Combine the Gear, Country, Taxa and Spatial Cells with the Catch files, split into industrial and non-industrial catch.
2. Save each year of data as a separate file into: "globalprep/prs_fish/v2020/int/annual_catch"

**Function for Combining & Saving Files**: Create functions to read in each file name, filter for type of catch, combine with gear, country, taxa, and spatial cells, and save each year of catch data into a separate file in mazu.


**Non-Industrial Catch Data**
```{r tidy, eval=FALSE}
## Set function to read in each file, filter for type of catch, combine with gear, country, taxa, and spatial cells, and save each year of data back into mazu
combine_fis_NInd <- function(x) {
  #x <- file.path(rawFolder, "Catch1950_1954.rds")
 
  ## Read in the catch data
  ## Create a total Landings column
  ## Join with master and spatial cells file
  df <- readRDS(x) %>%
    filter(ReportedIND == 0, IUUIND==0, DiscardsIND == 0) %>%
    rename("Reported" = "ReportedNIND", "IUU" = "IUUNIND", "Discards" = "DiscardsNIND") %>%
    dplyr::select(-ReportedIND, -IUUIND, -DiscardsIND) %>%
    dplyr::mutate(Landings = Reported+IUU) %>% 
    dplyr::left_join(country, by = c("CNumber" = "Cnumber")) %>%
    dplyr::left_join(taxa, by = "Taxonkey") %>% 
    dplyr::left_join(gear, by = c("FGearCode")) %>%
    dplyr::left_join(spatialCells, by = "Cell") %>%
    dplyr::select(Cell, Reported, IUU, Discards, Landings, Year = IYear, CountryName = `FAO name`, TaxonName, CommonName, FleetGearName, LonCentre, LatCentre, OceanAreasqkm)
  ...
  for(yr in five_years){
    ...
    ind_Nind <- basename(x) %>% 
      ...
      str_remove("h.*") # remove everything after the first underscore
      
    
    write_rds(single_yr_df, paste0(dir_M,"/git-annex/globalprep/prs_fish/v2020/int/annual_catch/", "NInd", ind_Nind, "h", "_",  yr, ".rds"))
    
  }
  
  }

#combine_fis_NInd(file.path(rawFolder,"Catch1950_1954.rds")) #check to see if function writes to mazu


#df <- readRDS(file.path(dir_M,"/git-annex/globalprep/prs_fish/v2020/int/annual_catch/NIndCatch_1950.rds")) # check actual file
```

Create list of industrial catch file names and apply the `combine_fis_NInd` function to save each individual 5-year interval catch data.

```{r, eval=FALSE}

Nind_files <- dir(file.path(dir_M,"git-annex/globalprep/_raw_data/IMAS_GlobalFisheriesLandings/d2020"), pattern ="Catch", full.names=TRUE)


## Wrangle and Save Each Year of Data Separately
NindCatch <- map_df(Nind_files, combine_fis_NInd)

#check to see if function worked properly
#df <- readRDS(file.path(dir_M,"/git-annex/globalprep/prs_fish/v2020/int/annual_catch/NIndCatch_2017.rds"))
```
...
combine_fis_Ind <- function(x) {
  ...
  ## Join with master and spatial cells file
  df <- readRDS(x) %>%
    filter(ReportedNIND == 0, IUUNIND==0, DiscardsNIND == 0) %>%
    rename("Reported" = "ReportedIND", "IUU" = "IUUIND", "Discards" = "DiscardsIND") %>%
    dplyr::select(-ReportedNIND, -IUUNIND, -DiscardsNIND) %>%
    dplyr::mutate(Landings = Reported+IUU) %>% 
    dplyr::left_join(country, by = c("CNumber" = "Cnumber")) %>%
    dplyr::left_join(taxa, by = "Taxonkey") %>% 
    dplyr::left_join(gear, by = c("FGearCode")) %>%
  ...
  for(yr in five_years){
    ...
    write_rds(single_yr_df, paste0(dir_M,"/git-annex/globalprep/prs_fish/v2020/int/annual_catch/", "Ind" ,ind_Nind, "h", "_",  yr, ".rds"))
    
  }
  
  }

#combine_fis_Ind(file.path(rawFolder,"Catch1950_1954.rds")) #check to see if function writes to mazu


#df <- readRDS(file.path(dir_M,"/git-annex/globalprep/prs_fish/v2020/int/annual_catch/IndCatch_1950.rds")) # check actual file
```


Create list of non-industrial catch file names and apply the `combine_fis_Ind` function to save each individual 5-year interval catch data. Then, create a single file that has all years of data.

```{r, eval=FALSE}

ind_files <- dir(file.path(dir_M,"git-annex/globalprep/_raw_data/IMAS_GlobalFisheriesLandings/d2020"), pattern ="Catch", full.names=TRUE)

## Wrangle and Save Each Year of Data Separately
indCatch <- map_df(ind_files, combine_fis_Ind)

#check to see if function worked properly
df <- readRDS(file.path(dir_M,"/git-annex/globalprep/prs_fish/v2020/int/annual_catch/IndCatch_2017.rds"))
```
...
raw_suffx <- list(nindcatch = "NIndCatch", indcatch = "IndCatch")
rast_prefx <- list(artisanal = "artisanal", comm = "comm")
...
#  dplyr::mutate(difference = Cell - lag)
  ...
  ## Rasterize Catch: swapping template cell values with those in dataframe
    # test <-  raster::subs(cells_raster, landings, by = "Cell", which = "cell_catch", subsWithNA=TRUE)
    # plot(test)
  # test <-  raster::subs(cells_raster, landings, by = "Cell", which = "cell_catch", subsWithNA=TRUE, filename = paste0(rastFolder, "artisanal", '_landings/annual_catch_', yr ,'.tif'), overwrite=TRUE)
 #  plot(test)
 #  test_ly <- raster(file.path("/home/shares/ohi/git-annex/globalprep/prs_fish/v2019/int/artisanal_landings/annual_catch_2011.tif"))
 #  plot(test_ly)
...
```{r, eval=FALSE}
#### STOPPED HERE APRIL 6 ####
## Read in rasters for commercial landings between 2000 and 2017
...
allRasters <- raster::stack(commercial[2:19])
...
log_Catch <- raster::calc(tot_Catch, function(x){log(x+1)},
             filename = paste0(rastFolder,"datacheck/commLand_2000_2017_log.tif"), overwrite=TRUE)


## Plot & Save raster
png(filename="figs/log_IndCatch_2000_2017.png")
...
## Read in rasters for artisanal landings between 2000 and 2017
...
## Compare 2000-2015 industrial catch (log-transformed) to Watson figure
## create a raster stack from the input raster files (exclude 1999 which is commercial[1])
allRasters <- raster::stack(artisanal[2:19])
...
## apply log to adjust for really large values
log_Catch <- raster::calc(tot_Catch, function(x){log(x+1)},
             filename = paste0(rastFolder,"datacheck/artisanalLand_2000_2017_log.tif"), overwrite=TRUE)


## Plot & Save raster
png(filename="figs/log_NIndCatch_2000_2017.png")
plot(log_Catch, col=cols, alpha=1)
dev.off()
...
* Prepare and gapfill NPP data in [**npp.Rmd**](https://mazu.nceas.ucsb.edu/rstudio/files/github/ohiprep_v2020/globalprep/prs_fish/v2020/npp.html); 
...
npp_files_gf <- list.files(file.path(dir_M,"git-annex/globalprep/prs_fish/v2020/VGPM_primary_productivity/int/annual_npp"), full.names=TRUE, pattern = "gf")
...
catch_npp_fun <- function(file, layer){ # file = dem_d_files[1]  layer = 'dem_dest'
  ...
  projectRaster(catch, npp, method = 'ngb', over=TRUE, filename = file.path(dir_M,"git-annex/globalprep/prs_fish/v2020/tmp/temp_resample2-1.tif"), overwrite=TRUE) 
  
  catch_resmp <- raster(file.path(dir_M,"git-annex/globalprep/prs_fish/v2020/tmp/temp_resample2-1.tif"))
    
  overlay(catch_resmp, npp, fun=function(x,y){x/y}, filename = file.path(dir_M, sprintf("git-annex/globalprep/prs_fish/v2020/int/%s/annual_catch_corr_%s.tif",layer,yr)), overwrite=TRUE)
...
num <- 10 # change to check different raster file in the list
...
# years_of_data[[1]]:years_of_data[[length(years_of_data)-4]] # goes until 2013

foreach (i = 2003:2013) %dopar%{  #i = 2010
  ...
  ## spot check some of the data... looks like a lot more activity in Greenland last year.. 
 # plot(raster(file.path(rastFolder, "comm_landings/mean_catch_2010_2014.tif")))
 #    plot(raster(file.path("/home/shares/ohi/git-annex/globalprep/prs_fish/v2019/int/", "comm_landings/mean_catch_2010_2014.tif")))
...
num <- 11 # change to check different raster file year
...
ref_clb <- quantile(vals, prob = 0.9999, na.rm=T)  # 10.64 - 2018; 11.80793 - 2019, 12.1 - 2020
...
ref_chb <- quantile(vals, prob = 0.9999, na.rm=T)  # 9.88 - 2018; 9.480187 - 2019, 10.1 - 2020
...
ref_alb <- quantile(vals, prob = 0.9999, na.rm=T)  # 10.72 - 2018; 12.38605 - 2019, 11.3 - 2020
```
...
ref_ahb <- quantile(vals, prob = 0.9999, na.rm=T)  # 7.45 - 2018; 9.303356 - 2019, 7.67 - 2020
...
Note: all quantiles except for commercial high bycatch increased slightly from last year's assessment (see [v2020 prep](https://github.com/OHI-Science/ohiprep_v2020/blob/master/globalprep/prs_fish/v2020/fishing_pressure_layers.Rmd)).
...
foreach (i = 2003:2013) %dopar%{ # i = 2011
...
# compare to v2019 -- mostly looks the same. It seems there is a lot more artisanal fishing in the new data (2020 data that is)
# cl <- raster(file.path('/home/shares/ohi/git-annex/globalprep/prs_fish/v2019/int/', 'comm_landings/mean_catch_1km_2011_2015.tif'))
# cd <- raster(file.path('/home/shares/ohi/git-annex/globalprep/prs_fish/v2019/int/', 'comm_discards/mean_catch_1km_2011_2015.tif'))
# al <- raster(file.path('/home/shares/ohi/git-annex/globalprep/prs_fish/v2019/int/', 'artisanal_landings/mean_catch_1km_2011_2015.tif'))
# ad <- raster(file.path('/home/shares/ohi/git-annex/globalprep/prs_fish/v2019/int/', 'artisanal_discards/mean_catch_1km_2011_2015.tif'))
# 
# all <- stack(cl,cd,al,ad)
# plot(all, col=cols, axes=FALSE ,box=FALSE)
...
foreach (i = 2003:2013) %dopar%{ # i = 2011
  ...
  mask(cl_mask, ocean, filename = paste0(dir_M, '/git-annex/globalprep/prs_fish/v2020/output/comm_low_bycatch/lb_fish_pressure_', yrs[1], '_', yrs[5], '.tif'), overwrite = TRUE)

  ## Commercial High Bycatch (Discards)
  cd_mask <- raster(paste0(rastFolder, 'comm_discards/mean_catch_1km_', yrs[1], '_', yrs[5], '.tif'))
  mask(cd_mask, ocean, filename = paste0(dir_M, '/git-annex/globalprep/prs_fish/v2020/output/comm_high_bycatch/hb_fish_pressure_', yrs[1], '_', yrs[5], '.tif'), overwrite = TRUE)


  ## Artisanal Low Bycatch (Landings)
  alb_mask <- raster(paste0(rastFolder, 'artisanal_landings/mean_catch_1km_', yrs[1], '_', yrs[5], '.tif'))
  mask(alb_mask, ocean, filename = paste0(dir_M, '/git-annex/globalprep/prs_fish/v2020/output/art_low_bycatch/alb_fish_pressure_', yrs[1], '_', yrs[5], '.tif'), overwrite = TRUE)

  ## Artisanal High Bycatch (Discards)
  ahb_mask <- raster(paste0(rastFolder, 'artisanal_discards/mean_catch_1km_', yrs[1], '_', yrs[5], '.tif'))
  mask(ahb_mask, ocean, filename = paste0(dir_M, '/git-annex/globalprep/prs_fish/v2020/output/art_high_bycatch/ahb_fish_pressure_', yrs[1], '_', yrs[5], '.tif'), overwrite = TRUE)
...
outFolder <-  "/home/shares/ohi/git-annex/globalprep/prs_fish/v2020/output"
...
old <- read.csv("../v2019/output/comm_lb.csv")

old_check1_comm_lb <- old_new %>%
  filter(is.na(pressure_score), new_pressure_score >=0 ) # 0 new regions w data
old_check2_comm_lb <- old_new %>%
...
old <- read.csv("../v2019/output/comm_hb.csv") %>%
  filter(year == 2014) %>% 
  left_join(new, by = 'rgn_id')
...
old <- read.csv("../v2019/output/art_lb.csv") %>%
  filter(year == 2014) %>% 
  left_join(new, by = 'rgn_id')
...
old_check1_art_lb <- old %>%
  filter(is.na(pressure_score), new_pressure_score >=0 ) # 2 new regions w data
old_check2_art_lb <- old %>%
  filter(pressure_score >= 0 , is.na(new_pressure_score)) # 10 regions now with NA that previously weren't

## Artisanal High Bycatch pressure 
new <- read.csv("output/art_hb.csv") %>% 
  filter(year == 2014) %>% 
  dplyr::select(rgn_id, new_pressure_score = pressure_score) 

old <- read.csv("../v2019/output/art_hb.csv") %>% 
...
old_check3_art_hb <- old %>%
  mutate(pressure_diff = pressure_score - new_pressure_score) 
```
...
##                        - 4/15/20 not much change from year to year within same assessment year
new <- read.csv("output/comm_lb.csv") %>%
  filter(year == 2014) %>%
  dplyr::select(rgn_id, new_pressure_score = pressure_score) 
