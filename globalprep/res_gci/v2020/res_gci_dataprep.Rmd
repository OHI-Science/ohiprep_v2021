---
title: "OHI 2020 - Data Prep for Global Competitiveness Index (GCI) Resilience"
...
**Reference**: http://reports.weforum.org/global-competitiveness-report-2019/downloads/

**Downloaded**: 06/29/2020

**Description**:
A weighted index based on 12 socioeconomic factors relevant to economic competitiveness and productivity: institutions, infrastructure, macroeconomic environment, health and primary education, higher education and training, goods market efficiency, labor market efficiency, financial market development, technological readiness, market size, business sophistication, and innovation. Report on the 2019 global competitiveness index is located [here](http://www3.weforum.org/docs/WEF_TheGlobalCompetitivenessReport2019.pdf), and data can be downloaded [here](http://reports.weforum.org/global-competitiveness-report-2019/downloads/).
...
dir_wef  <- list.files(file.path(path_raw_data, sprintf("WEF-Economics/%s", data_yr_gci)), 
                       pattern = "WEF_GCI_4.0_2019_Dataset.csv$", 
                       full.names = TRUE)
wef_raw <- read.csv(dir_wef, skip = 4, check.names = FALSE, stringsAsFactors = FALSE)

dir_wef # check reading the correct year of downloaded data
```

NOTE FROM GAGE: I stopped editing stuff here. The rest will have to be updated 
...
wef <- wef %>%
  ...
  dplyr::select(-(1:2), -(4:8), year = Edition) %>%
  ...
  dplyr::select(year, country, score) %>%
  filter(year >= 2010)
```
...
wef_gf <- wef_gf %>%
  ...
  dplyr::select(year, country, score, gapfill, method)
