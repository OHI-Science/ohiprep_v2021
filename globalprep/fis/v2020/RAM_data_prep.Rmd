---
title: "OHI 2020: Food Provision/Fisheries, Preparing RAM B/Bmsy data"
...
#   dplyr::select(stockid,stocklong,year,ram_bmsy)
# 
# new_species_list <- data.frame(stockid = sort(setdiff(ram_bmgt_new_final$stockid, ram_bmsy_new$stockid))) %>%
...
**Also only needed for 2019 assessment:**
...
- 313 of these stocks have at least 1 year of gapfilled data.  
...
ram_bmsy_gf <- ram_bmsy_new %>%
  filter(year >= 2001 & year <= 2016) %>%   # 2015 corresponds to the final year of Watson catch data
  group_by(stockid) %>%
...
sum(table(tmp$gapfilled))  # 222 stocks have at least one year of B/Bmsy values gapfilled - v2019
                          # 313 stocks have at least one year of B/Bmsy values gapfilled; this is because there is an additional year of data in RAM... 2016 - v2020
...
summary(ram_bmsy_gf) #892 NAs for ram_bmsy
sum(ram_bmsy_gf$ram_bmsy_predict < 0 )  # 21 of the predicted B/Bmsy values go below zero.  
...
tmp2 <- sort(setdiff(wat_sp$wat_scientificname, ram_sp$scientificname)) # 1210 species names
