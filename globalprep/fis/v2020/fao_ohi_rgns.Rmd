...
# See the mispelling for CODIf-XIV vs COD1f-XIV. Let's check to make sure that these didn't actually change across the years, and fix it in the data. 
# 
# ram_bmsy_new_check <- ram_bmsy %>% 
#   filter(stockid == "COD1f-XIV") ## 39 obs
# 
# ram_bmsy_old_check <- ram_bmsy_old %>% 
#   filter(stockid == "CODIf-XIV") ## 39 obs
# 
# #they have the same amount of observations, but it looks like the bmsy values are updated for the new year. I think we should use the new ones. For now, we will filter out the new ones, and deal with this problem in RAM_data_prep.Rmd to see if they are double counting the bmsy values. We will just use the same fao ohi ids that we have found in the past, since those won't have changed. 
# 
# ram_bmsy <- ram_bmsy %>%
#   filter(stockid != "COD1f-XIV")
# ## check number of unique entries
# new_stockid <- unique(ram_bmsy$stockid) # 442 unique entries now
# 
# ## find newly added stocks, when using timeseries_values_views
# newStocks <- setdiff(new_stockid, old_stockid) # there are 4 new stocks
# unusedStocks <- setdiff(old_stockid, new_stockid) # these are no longer included in the RAM data, will be deleted
...
## v2020... after some investigating it was found that there are a ton of duplicates in this file. This is a mistake. Every stock should only have one unique combo of each rgn id and fao id. We need to isolate and remove these. First we will remove the easy ones using distinct(). 
...
```{r}
#### 2020 fix only ####
# for some reason, a couple of the stocks that were already present are showing up as na for FAO and RGN ID. Spot check each of the stocks that show up in RAM_filt with RAM_rgns_new. We need to make sure that each stock only shows up once unless it is in multiple FAO/ohi regions. 
...
# i will go in and fix these myself in excel, and re upload. I deleted the two NA rows for PERCHQCI and OROUGHYNWCR. There were also two mispelling in there that i fixed for these two.
...
write.csv(RAM_fao_ohi_rgn, "int/RAM_fao_ohi_rgns_final.csv", row.names=FALSE)
```
