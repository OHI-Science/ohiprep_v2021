...
write.csv(roll_prod, "int/tonnes_coral_harvest.csv")
```

## Calculate pressure score
 - Divide the harvest by the area of coral and take the 95th quantile of harvest 
 - Take a harvest weighted mean of harvest intensity and health to get the pressure. 
```{r}
## read in coral extent 
coral_ext <- read_csv("../../hab_coral/v2012/data/habitat_extent_coral_updated.csv") %>%
...
write.csv(smooth_prod, "int/smooth_prod_coral.csv", row.names = FALSE)
...
prs_coral_harvest <- smooth_prod %>%
  dplyr::select(rgn_id, year, pressure_score = tonnes_rel)
write.csv(prs_coral_harvest, "output/prs_coral_harvest_scores_1.csv", row.names = FALSE)



```

## Create exposure and risk layers for risk (so that we can calculate sustainability)

```{r}
# ### calculates NP exposure based on habitats (corals).
...
#     #########################################.
#     
# coral_harvest_tonnes <- read_csv("int/smooth_prod_coral.csv") %>%
#   dplyr::select(year, rgn_id, product, tonnes) 
# 
# coral_harvest_rel <- read_csv("output/prs_coral_harvest_scores_1.csv") %>%
#   rename("tonnes_rel" = "pressure_score")
# 
# coral_harvest <- left_join(coral_harvest_tonnes, coral_harvest_rel)
# 
...
#     write.csv(gap_fill, "int/prs_exposure_corals_gf.csv", row.names = FALSE)
#     
#     ### add exposure for countries with (habitat extent == NA)
#     exp_corals <- exp_corals %>%
#       dplyr::group_by(product) %>%
...
#     
# ##save exposure
#     write.csv(exp_corals, "int/prs_exposure_corals.csv", row.names = FALSE)
#     
# 
...
#     prs_coral_risk <-
#       expand.grid(
#         region_id  = unique(coral_harvest$rgn_id),
#         year = unique(coral_harvest$year)
#       ) %>%
#       mutate(corals = 1) %>%       ### corals
#       gather(product, risk,-region_id,-year) %>%
#       mutate(product = as.character(product))
#     
#     prs_coral_risk_gf <- prs_coral_risk %>%
#       mutate(gapfilled = 0, method = NA) %>%
#       dplyr::select(-risk)
#     write.csv(prs_coral_risk, "int/prs_coral_risk_gf.csv", row.names = FALSE)
#   
# ### calculates coral sustainability coefficient based on
...
#     
# ### coral product_status:
# ### [rgn_id  rgn_name  product  year sustainability  product_status]
# 
#         prs_coral_sust <- exp_corals %>%
#       left_join(prs_coral_risk, by = c("rgn_id" = 'region_id', 'year', 'product')) %>%
