...
2020
Using updated April 2020 data
Made a slight change to the calc_area function by moving the `ungroup()` step one step earlier, to avoid error
Got rid of ggplotly funtion, simply called the comparison graph
...
# Data Source
**Reference**: IUCN and UNEP-WCMC (2020), The World Database on Protected Areas (WDPA) [On-line], April 2020. Cambridge, UK: UNEP-WCMC. Available at: www.protectedplanet.net.

**Downloaded**: April 1, 2020
...
**File location**: `Mazu:git-annex/globalprep/_raw_data/wdpa_mpa/d2020/WDPA_Apr2020-shapefile/`
...
dir_goal      <- file.path(here('globalprep',goal, scenario)) #changed this 2020, took out global prep
...
rgn_eez <- region_data() ##added this
...
* 2020 AY data goes through March (downloaded on April 1) 2020
...
status_3nm_old <- read_csv(file.path(dir_goal, '../v2019/output', 'lsp_prot_area_offshore3nm.csv')) %>%
  full_join(read_csv(file.path(dir_goal, 'output', 'rgn_area_offshore3nm.csv')),
            by = 'rgn_id') %>%
...
status_1km_old <- read_csv(file.path(dir_goal, '../v2019/output', 'lsp_prot_area_inland1km.csv')) %>%
  full_join(read_csv(file.path(dir_goal, 'output', 'rgn_area_inland1km.csv')),
            by = 'rgn_id') %>%
...
lsp_status_plot <- ggplot(lsp_new_old, 
  ...
  labs(x = 'LSP status v2019 (data through March 2020)',
       y = 'LSP status v2020 (data through March 2020)',
       title = 'Comparing LSP status: 2020 vs 2019') +
  facet_wrap( ~ rgn)

lsp_status_plot #got rid of ggplotly

ggsave(file.path(dir_goal, 'Figs/plot_v2019_v2020.png'), 
...
write.csv(x, "output/major_changes_2020.csv" ) #added this so I could come back to it without rerunning everything
```
