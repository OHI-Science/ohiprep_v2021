...
shp_raw     <- file.path(dir_data, 'WDPA_Apr2020-shapefile', 'WDPA_Apr2020-shapefile-polygons')
shp_reorder <- file.path(dir_data, 'shps', 'WDPA_Apr2020_shp_ordered')
...
if(!file.exists(paste0(shp_reorder, '.shp'))) {
  ...
  ## filter polygons
  # glimpse(wdpa_poly@data) ## check class of status_yr: checked, integer
  wdpa_poly <- wdpa_poly %>%
  ...
  ## [1] "Designated"  "Proposed"  "Inscribed" "Not Reported" "Adopted" "Established"     

  #x <- wdpa_poly@data$status %>% filter(str_detect(tolower(mang_plan), 'non-mpa program'))
  ## 84 observations
  ##update: when I run, I only get 44? but also I have to get rid of the @data for it to work
...
shp_xformed <- file.path(dir_data, 'shps', 'WDPA_Apr2020_shp_xformed')
...
rast_wdpa_file   <- file.path(dir_goal_anx, 'rast', 'wdpa_2020_moll_500m.tif')

if(!file.exists(rast_wdpa_file)) {
shp_xformed_file <- file.path(dir_data, 'shps', 'WDPA_Apr2020_shp_xformed')
...
#From 2020;
# Reading layer `WDPA_Apr2020_shp_xformed' from data source `/home/shares/ohi/git-annex/globalprep/_raw_data/wdpa_mpa/d2020/shps' using driver `ESRI Shapefile' -->
# Simple feature collection with 222501 features and 18 fields
# geometry type:  MULTIPOLYGON
# dimension:      XY
# bbox:           xmin: -17784490 ymin: -8846134 xmax: 17827600 ymax: 8756123
# CRS:            unknown
# elapsed:  55.604fasterize elapsed:  27.485

```

## Data Checking

Compare shapfile 2019 v 2020.
...
## Read in as sf
wdpa_sf <- st_read(dsn = file.path(dir_data, 'shps'), layer = "WDPA_Apr2020_shp_ordered")
wdpa_sf_19 <- st_read(dsn = file.path(dir_data, '../d2019', 'shps'), layer = 'WDPA_Apr2019_shp_ordered')

plot(wdpa_sf[1])
plot(wdpa_sf_19[1])

#to click around and look at different values to check differences can use (kind of clunky)
#click(wdpa_sf)
...
## Filtering for export (added due to server issues in 2020)
Filtering out countries with large changes to look at in Arc, since rationale for these changes was difficult to find through news or research
```{r}
...
#2020
wdpa_rus_20 <- wdpa_sf %>%
  select(parent_iso, geometry) %>% 
  filter(parent_iso == "RUS")
#2019
wdpa_rus_19 <- wdpa_sf_19 %>%
  select(parent_iso, geometry) %>% 
  filter(parent_iso == "RUS")

st_write(wdpa_rus_20, dsn = dirname(file.path(dir_goal,"v2020/int")), layer = "wdpa_rus_20.shp", driver = "ESRI Shapefile")

st_write(wdpa_rus_19, dsn = dirname(file.path(dir_goal,"v2020/int")), layer = "wdpa_rus_19.shp", driver = "ESRI Shapefile")
...
wdpa_hti_20 <- wdpa_sf %>%
  select(parent_iso, geometry) %>% 
  filter(parent_iso == "HTI")
#2019
wdpa_hti_19 <- wdpa_sf_19 %>%
  select(parent_iso, geometry) %>% 
  filter(parent_iso == "HTI")

st_write(wdpa_hti_20, dsn = dirname(file.path(dir_goal,"v2020/int")), layer = "wdpa_hti_20.shp", driver = "ESRI Shapefile")

st_write(wdpa_hti_19, dsn = dirname(file.path(dir_goal,"v2020/int")), layer = "wdpa_hti_19.shp", driver = "ESRI Shapefile")
...
wdpa_moz_20 <- wdpa_sf %>%
  select(parent_iso, geometry) %>% 
  filter(parent_iso == "MOZ")
#2019
wdpa_moz_19 <- wdpa_sf_19 %>%
  select(parent_iso, geometry) %>% 
  filter(parent_iso == "MOZ")

st_write(wdpa_moz_20, dsn = dirname(file.path(dir_goal,"v2020/int")), layer = "wdpa_moz_20.shp", driver = "ESRI Shapefile")

st_write(wdpa_moz_19, dsn = dirname(file.path(dir_goal,"v2020/int")), layer = "wdpa_moz_19.shp", driver = "ESRI Shapefile")
...
wdpa_com_20 <- wdpa_sf %>%
  select(parent_iso, geometry) %>% 
  filter(parent_iso == "COM")
#2019
wdpa_com_19 <- wdpa_sf_19 %>%
  select(parent_iso, geometry) %>% 
  filter(parent_iso == "COM")

st_write(wdpa_com_20, dsn = dirname(file.path(dir_goal,"v2020/int")), layer = "wdpa_com_20.shp", driver = "ESRI Shapefile")

st_write(wdpa_com_19, dsn = dirname(file.path(dir_goal,"v2020/int")), layer = "wdpa_com_19.shp", driver = "ESRI Shapefile")
