...
* Tourism sustainability data from the WEC Travel and Tourism Competitiveness Report were updated to include the September 2019 report update. 
* Proportion of jobs in tourism - WTTC data reported until 2030, but 2020 is most recent year of real data (year_max) (downloaded from WTTC on 07/08/2019)
...
# v2020: Adding the low pop df identifies 13 additional regions that should be NA instead of gapfilled, taking the total number of NAs in the data set from xxx to 832
...
weight_data <- data.frame(country = c("China", "Hong Kong SAR"),
                          population = c(1386000000, 7392000))
...
The Artisanal Opportunities goal uses gdppcppp data, so we will get the data that was processed for that goal. (NOTE: Just use the AO data from last year for now)


```{r worldbank, eval=FALSE}
wb <- read.csv("../../ao/v2019/intermediate/gdppcppp_ohi.csv") %>%
...
population_weights <- data.frame(country = c("Virgin Islands", "Puerto Rico",
                                 ...
                                 population = c(107268, 3193694,
                                         1386000000, 7392000, 622567,
                                         168775, 55144))
