...
## Travel warnings
Travel warning will be excluded from the 2020 global assessment. 

## Tourism Sustainability 
There is new data available for this. 

**We were able to update the following data:**
* Tourism sustainability data from the WEC Travel and Tourism Competitiveness Report were updated for the 2020 assessment. 

* Proportion of jobs in tourism - WTTC data reported until 2030, but 2020 is most recent year of real data (year_max) (downloaded from WTTC on )
...
tr_data_nas <- tr_data_raw_gf %>% 
  filter(is.na(Ep_all)) %>% 
  dplyr::select(rgn_id, year, r1_label, r2_label, rgn_label) %>% 
...
tr_data <- tr_data_raw_gf %>%
  dplyr::select(rgn_id, year, Ep=Ep_all) 
...
old <- read_csv(here('globalprep/tr/v2019/output/tr_jobs_pct_tourism.csv')) %>%
  dplyr::select(rgn_id, year, ep_old=Ep)
