...
# Summary
The habitat pressure data layer is created from fish catch by gear type of industrial and non industrial fishing provided by Watson (2018). Total tonnes of catch are calculated for each OHI region and then standardized by area (km^2) soft-bottom habitat in each region. These values are then rescaled to be between 0 and 1. 
...
## Find 95th quantile density for each year
quant_95 <- data_density %>%
  group_by(year) %>%
  summarize(quantDensity = quantile(density, c(0.95), na.rm=TRUE)) %>%
  data.frame()

write.csv(quant_95, "int/reference_point_quant_95.csv", row.names=FALSE) # Make sure you are working in the correct working directory
...
ref_point <- read.csv("int/reference_point_quant_95.csv")
ref_point_median <- median(ref_point$quantDensity) # v2020: 20.5649376263763
