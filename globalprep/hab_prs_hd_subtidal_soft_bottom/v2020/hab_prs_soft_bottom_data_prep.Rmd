...
Data reports catch by gear type. For each gear type we assign a multiplier according to how much the gear affects the sea bottom. The table with the multipliers is shown below (1 is extremely harmful and 0 is not harmful):
...
test <- raster(file.path(dir_M, ('git-annex/globalprep/hab_prs_hd_subtidal_soft_bottom/v2020/int/catch_count_2015.tif')))
...
#setwd("~/github/ohiprep_v2020/globalprep/hab_prs_hd_subtidal_soft_bottom/v2020") 
...
## Reference point = max values across all years
...
# ref_point 2020 = 4842.57385 (year 2015)
...
old <- read.csv("../v2019/output/habitat_health_softbottom.csv") %>%
...
compare_plot <- ggplot(data = test, aes(x = old_health, y = health, label = rgn_id)) + # "test" is a RasterLayer class so this doesn't run
  geom_point()+
...
condition_pressure <- data_rescaled_2 %>%
  ...
  mutate(pressure = 1 - density_rescaled_median_capped) %>% # Transforms it back into pressure-measure
...
## Health data
old_health <- read.csv("../v2019/output/habitat_health_softbottom.csv") %>%
  dplyr::filter(year == 2014) %>% 
  dplyr::select(rgn_id, old_health=health)
...
## Pressure
old_pressure <- read.csv("../v2019/output/hd_sb_subtidal.csv") %>%
  dplyr::filter(year == 2014) %>% 
...
## Trend
old_trend <- read.csv("../v2019/output/habitat_trend_softbottom.csv") %>%
  dplyr::filter(year == 2014) %>% 
