...
sw_sus_rgn <- sw_sus_rgn %>% 
  mutate(match_type = case_when(!is.na(country) ~ "sw_sp_c", # Add match type specific to species/country
                                str_detect(report_title, regex("Global")) ~ "sw_sp_g" # Add match type specific to species/global
                                ))
...
## Join taxa specific to country/species/environment
c_sp_env_sw = sw_sus_rgn %>% 
  filter(match_type == 'c_sp_env_sw') %>% 
  select(rgn_id, species=species_fao, environment, Sust_c_sp_env = Maric_sustainability)
