...
sw_sus_rgn <- name_2_rgn(df_in = sw_sus, 
                       fld_name='country', 
                       flds_unique=c('water_body', 'fao_species', 'sw_species'),
                       keep_fld_name = TRUE)
## Goes from 330 obs. to 127 obs. (do we want to include ones that don't have country listed?)

## Duplicates found: 1) "Canada", 2) "United States of America", 3) "Indonesia", 4) "Viet Nam", 5) "Myanmar", 6) "Colombia", 7) "China", 8) "Malaysia", 9) "Egypt", 10) "New Zealand", 11) "Thailand"
  # Can ignore since we're concerned with the sustainability scores for the species, not the countries
...
**Check that the non-matches between Seafood Watch sustainability FAO spp (sw_sus_rgn) and the FAO mariculture species in the wrangled FAO Aquaculture Production data table (maric) are not due to spelling errors or slightly different names. We want to include as many species that have sustainability scores as possible**
...
```{r gapfill-sw-sus-data}

# add column "match_type" and assign based on region/country first
  # then if there's a global value, fill in that for each region that has that species
  # for species that don't have anything, explore genus/family info and subregion info; fuss around
  # for subregions, start with UN geopolitical georegions; see if there's any relationships
  # look at genus/family level globally vs subregions
...
Append Seafood Watch sustainability scores to the FAO mariculture data.

The following joins the Seafood Watch sustainability scores to regions/species that have Seafood Watch data.

```{r add-sw-sus-fao-data}

## Join taxa specific to species/country
sw_sp_c = sw_sus_rgn %>% 
...
## Join taxa specific to species/global Seafood Watch data
sw_sp_g = gf_sw_sp_g %>% 
...
## Join taxa specific to species/water body
sw_sp_w = sw_sus_rgn %>% 
...
summary(mar_sw_sus)
# Sust_sw_sp_g and Sust_sw_sp_w didn't have any matches because they have no rgn_ids associated with them

```

Merge the three Seafood Watch type categories into a single sustainability score column in the following order:

1. Sust_sw_sp_w: specific to species/water body
2. Sust_sw_sp_g: specific to species/global Seafood Watch data
3. Sust_sw_sp_c: specific to species/country

For example, if Sust_sw_sp_w is missing, use Sust_sw_sp_g and so on. 

```{r sw-sus-merge}

mar_sw_sus = mar_sw_sus %>% 
  mutate(Sust = ifelse(!is.na(Sust_sw_sp_w), Sust_sw_sp_w, Sust_sw_sp_g)) %>%
  mutate(Sust = ifelse(is.na(Sust), Sust_sw_sp_c, Sust)) %>%
  select(-Sust_sw_sp_w, -Sust_sw_sp_g, -Sust_sw_sp_c)
...
## Select observations gapfilled at the species/genera level:
gf_sp_sw_sus <- sw_sus_rgn %>% 
  filter(gapfill != "actuals" & match_type == "species") %>%
  select(species, gapfill, Sust_gf_sp = Maric_sustainability)
