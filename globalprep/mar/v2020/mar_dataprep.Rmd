...
spp_all <- mar_sp_clean %>%
    .$spp_clean ## get a list of the species names to feed into the loop
...
write_csv(spp_fam_tmp, chunk_file) ## write to a temp folder
  ...
  spp_family_df <- lapply(spp_countries_chunk_files, FUN = function(x) {
    read.csv(x)}) %>%
    bind_rows()
  
  #split into NAs and re run through loop again with updated names... i.e. remove "nei" from spp names
  
  write.csv(spp_family_df, "int/taxize_run_1.csv", row.names = FALSE)
#test <- tax_name('Boleophthalmus pectinirostris', get = "family") #test to see if it will work
...
gf_sw_sp_g <- sw_sus_rgn %>% 
  ...
  add_row(rgn_id = 180, rgn_name = 'United Kingdom', species = 'Atlantic cod', score = 6.61, match_type = 'sw_sp_g', gapfill = 'sp_global', .after = 1) %>% # Source: http://www.fishbase.us/report/fao/FAOAquacultureList.php?scientific=Gadus+morhua
    add_row(rgn_id = 73, rgn_name = 'Russia', species = 'Atlantic cod', score = 6.61, match_type = 'sw_sp_g', gapfill = 'sp_global', .after = 1) %>% # Source: http://www.fishbase.us/report/fao/FAOAquacultureList.php?scientific=Gadus+morhua
