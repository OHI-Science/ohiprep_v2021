---
title: 'OHI 2020: Natural Products'
...
[REFERENCE RMD FILE](https://cdn.rawgit.com/OHI-Science/ohiprep_v2020/master/globalprep/np/v2020/np_dataprep.html)

# Summary
This analysis converts FAO commodities data into data layers used to calculate OHI 2019 global natural products scores.  

# Updates from previous assessment
New year of FAO data (1976-2017). To stabilize the model, we include only commodities with >= 4 non-zero/non-NA values within the most recent 10 years (vs. the entire time frame we previously used).

For this year, consider only including commodities with USD value > 1.  This may further improve the stability of the model. Also want to narrow the products to just fish oil and seaweed to mitigate model instability.
...
for (f in files){ # f = files[2]
  ...
  ## Products join: attach product categories from com2prod, and filter out all entries that do not match a product category.
  ## Note: commodity_lookup is user-defined function to compare commodities in data vs commodities in lookup table
  ...
  ## Special case: user-defined function deals with breaking up Antilles into separate reported rgns
...
# v2020 unmatched: eswatini, palestine, yugoslavia (all of these are landlocked)  
...
## Read in quant dataset from int folder
h_tonnes <- read.csv(here('globalprep/np/v2020/int/tonnes.csv'))

## Read in value dataset from int folder
h_usd <- read.csv(here('globalprep/np/v2020/int/usd.csv'))

## Concatenates h_tonnes and h_usd data
## commodities includes rgn_name, rgn_id, commodity, product, year, tonnes, usd.
commodities <- h_usd %>%
...
## To see example below for Cook Islands
# filter(commodities, rgn_id==153 & year>2000)
...
## At this point, commodities includes: 
## rgn_id   rgn_name   commodity   product   year   usd   tonnes   gapfill
...
## Loads a dataframe identifying UN geogregions based on geopolitical data. Typically used for gapfilling.
UNgeorgn()

## Melanie's script to add a georegional ID tag based on country keys and IDs. Used to gapfill based on decreasing granularity. 
commodities <- commodities %>% 
  add_georegion_id() %>%
...
## Compare old and new (v2020)
h_x_usd_old <- read_csv(here("globalprep/np/v2019/int/np_harvest_usd_wide.csv"))
...
australia <- australia %>%
  ...
  tidyr::gather(key = "year", value = "value_v2020", -(rgn_name), -(commodity), -(rgn_id), -(product))

## Compare old and new for Australia
aus_compare <- aus_old %>% 
...
write.csv(h_x_tonnes, here('globalprep/np/v2020/int/np_harvest_tonnes_wide.csv'), row.names = FALSE, na = 'NA')
write.csv(h_x_usd, here('globalprep/np/v2020/int/np_harvest_usd_wide.csv'), row.names = FALSE, na = 'NA')
```
...
## Determine rolling averages for quant and value
roll_prod <- h_prod %>%
...
prod_weights <- peak_prod %>%
    filter(year==year_max) %>% # v2020: most recent year is 2017
    group_by(rgn_id) %>%
    mutate(
...
write.csv(smooth_prod, here('globalprep/np/v2020/int/np_harvest_smoothed_data.csv'), row.names = FALSE, na = '')

## Write individual data layers:
## Write NP weights layer also used to calculate pressures and resilience:
write.csv(prod_weights, here('globalprep/np/v2020/output/np_harvest_weights_from_usd.csv'), row.names = FALSE, na = '')

## Save tonnes data
tonnes <- smooth_prod %>%
  select(rgn_id, product, year, tonnes)

## Write tonnes data to .csv:
write.csv(tonnes, here('globalprep/np/v2020/output/np_harvest_tonnes.csv'), row.names = FALSE, na = '')

## Save relative tonnes data
tonnes_rel <- smooth_prod %>%
  select(rgn_id, product, year, tonnes_rel)

## Write relative tonnes data to .csv:
write.csv(tonnes_rel, here('globalprep/np/v2020/output/np_harvest_tonnes_rel.csv'), row.names = FALSE, na = '')
...
## Load in raw data from this year and last year:
new <- read.csv(here('globalprep/np/v2020/int/tonnes.csv'))
old <- read.csv(here('globalprep/np/v2019/int/tonnes.csv'))

## Bahamas (rgn #110)
new_bahamas <- filter(new, rgn_id==110) %>%
  arrange(commodity, year) %>% 
  rename(tonnes_new = "tonnes")
old_bahamas <- filter(old, rgn_id==110) %>%
  full_join(new_bahamas) %>%
  arrange(commodity, year) %>%
  filter(year>2000)

bahamas_compare <- old_bahamas %>% 
left_join(new_bahamas, by = c("commodity","year","rgn_id","rgn_name","product", "tonnes_new"))

plot(bahamas_compare$tonnes_new, bahamas_compare$tonnes)
abline(0,1, col="red")

# No changes at all in raw data, look into smoothed data: 

## Bahamas harvest smoothed data to see rel tonnes
smooth_new <- read.csv(here("globalprep/np/v2020/int/np_harvest_smoothed_data.csv")) %>%
  filter(rgn_id==110, year %in% c(2010:2016)) %>% 
  select(rgn_id, product, year, tonnes, tonnes_rel)
smooth_old <- read.csv(here("globalprep/np/v2019/int/np_harvest_smoothed_data.csv")) %>% 
  filter(rgn_id==110, year %in% c(2010:2016)) %>%
  select(rgn_id, product, year, tonnes, tonnes_rel) %>% 
...
new_tonnes <- read.csv(here("globalprep/np/v2020/output/np_harvest_tonnes.csv")) %>% 
  filter(rgn_id==110, year %in% c(2010:2016))
old_tonnes <- read.csv(here("globalprep/np/v2019/output/np_harvest_tonnes.csv")) %>% 
  filter(rgn_id==110, year %in% c(2010:2016)) %>% 
...
# Slight changes but still looks good
...
smooth_new <- read.csv(here("globalprep/np/v2020/int/np_harvest_smoothed_data.csv")) %>%
  filter(rgn_id==153, year %in% c(2010:2016)) %>% 
  select(rgn_id, product, year, tonnes, tonnes_rel)
smooth_old <- read.csv(here("globalprep/np/v2019/int/np_harvest_smoothed_data.csv")) %>% 
  filter(rgn_id==153, year %in% c(2010:2016)) %>%
  select(rgn_id, product, year, tonnes, tonnes_rel) %>% 
...
# Pretty big changes to smoothed data; due to FAO updating backfilling data or something else?

## Compare output files:
new_cook <- read.csv(here("globalprep/np/v2020/output/np_harvest_tonnes.csv")) %>% 
  filter(rgn_id==153, year %in% c(2010:2016))
old_cook <- read.csv(here("globalprep/np/v2019/output/np_harvest_tonnes.csv")) %>% 
  filter(rgn_id==153, year %in% c(2010:2016)) %>% 
  rename(tonnes_old = tonnes)
...
# Pretty big changes to final harvest tonnage data

### v2019 used the "gf_alternative_rough_notes.Rmd" to check gapfilling:
# Were they gapfilled?
new_tonnes_gf <- read.csv(here("globalprep/np/v2020/output/np_harvest_tonnes_gf.csv")) %>% 
  filter(rgn_id==153, year %in% c(2010:2016))

old_tonnes_gf <- read.csv(here("globalprep/np/v2019/output/np_harvest_tonnes_gf.csv")) %>% 
  filter(rgn_id==153, year %in% c(2010:2016))
...
# Some points were zero or 0.1 that are now NA 

## Lebanon harvest smoothed data to see rel tonnes
smooth_new <- read.csv(here("globalprep/np/v2020/int/np_harvest_smoothed_data.csv")) %>%
  filter(rgn_id==78, year %in% c(2010:2016)) %>% 
  select(rgn_id, product, year, tonnes, tonnes_rel)
smooth_old <- read.csv(here("globalprep/np/v2019/int/np_harvest_smoothed_data.csv")) %>% 
  filter(rgn_id==78, year %in% c(2010:2016)) %>%
  select(rgn_id, product, year, tonnes, tonnes_rel) %>% 
...
# A few slight outliers but not too concerning
...
# Fish oil data went from some tonnage to NA between v2019 and v2020
...
smooth_new <- read.csv(here("globalprep/np/v2020/int/np_harvest_smoothed_data.csv")) %>%
  filter(rgn_id==43, year %in% c(2010:2016)) %>% 
  select(rgn_id, product, year, tonnes, tonnes_rel)
smooth_old <- read.csv(here("globalprep/np/v2019/int/np_harvest_smoothed_data.csv")) %>% 
  filter(rgn_id==43, year %in% c(2010:2016)) %>%
  select(rgn_id, product, year, tonnes, tonnes_rel) %>% 
...
smooth_new <- read.csv(here("globalprep/np/v2020/int/np_harvest_smoothed_data.csv")) %>%
  filter(rgn_id==191, year %in% c(2010:2016)) %>% 
  select(rgn_id, product, year, tonnes, tonnes_rel)
smooth_old <- read.csv(here("globalprep/np/v2019/int/np_harvest_smoothed_data.csv")) %>% 
  filter(rgn_id==191, year %in% c(2010:2016)) %>%
  select(rgn_id, product, year, tonnes, tonnes_rel) %>% 
