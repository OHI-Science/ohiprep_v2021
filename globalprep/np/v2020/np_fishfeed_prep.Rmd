...
This is brand new to the 2020 assessment. Previously we used FAO commodities export dataset, but we refined our approach with actual catch tonnes, instead of export tonnes. 
...
forage <- read_csv(file.path(here(), "globalprep/np/v2020_new/raw/msleckman.61.1-CatchMSY_Nis_FAOAreas.csv.csv"))
...
watson_v3 <- read_csv(file.path(here(), "globalprep/np/v2020_new/raw/Codes_taxa.csv"))
...
watson_new_corr <- read_csv(file.path(here(), "globalprep/np/v2020_new/int/watson_new_corr.csv"))
...
write.csv(foragefish_list, here("globalprep/np/v2020_new/int/master_taxa_list_v5.csv"), row.names=FALSE)
...
foragefish_list <- read_csv(file.path("globalprep/np/v2020_new/int/master_taxa_list.csv"))
...
## Multiply by 0.70 to reflect the amount going to feed/oils

Justification from the [FAO](https://www.fisheries.noaa.gov/insight/feeds-aquaculture): About 70% of the fish meal and oil are produced from the harvest of small, open-ocean (pelagic) fish such as anchovies, herring, menhaden, capelin, anchovy, pilchard, sardines, and mackerel. These fish have short life cycles and are capable of rapid reproduction and stock replenishment.

```{r}
catch_fishfeed <- read_csv(file.path(here(),"globalprep/np/v2020_newint/watson_catch_forage_fish.csv"))

catch_non_human <- catch_fishfeed %>% 
  mutate(tons_non_human = tons*0.7)
...
write.csv(mean_catch_FOFM, "int/mean_catch_FOFM.csv", row.names = FALSE)
```
...
catch_FOFM <- read_csv(file.path(here(), "globalprep/np/v2020_new/int/mean_catch_FOFM.csv")) 
...
####
 ...
 penaltyTable <- data.frame(TaxonPenaltyCode = 1:6,
  ...
  write.csv(gap_fill_data, here('globalprep/np/v2020_new/output/NP_bbmsy_summary_gf.csv'), row.names = FALSE)
...
write.csv(score_data, here('globalprep/np/v2020_new/output/np_fofm_scores.csv'), row.names = FALSE)
