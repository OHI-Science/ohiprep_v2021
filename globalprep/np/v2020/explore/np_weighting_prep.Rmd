...
# Summary
This analysis converts FAO commodities tonnes data for ornamentals, Watson fisheries tonnes data for FOFM, and xxxx tonnes data for seasweeds into a weighting scheme per each ohi region. This weighting scheme will be applied to the scores when updating in ohi global. 
...
Import prepped FAO commodities data set and find a $USD / Tonne average value per product per region. Gapfill these based on the UN geopolitical region/global values. 
```{r}
...
global_values_df <- harvest_tonnes_usd_geo_rgn %>%
  ...
  mutate(values = ifelse(values == "Inf", NA, values)) %>% ## assign Inf as NA
  summarise(global_values = mean(values, na.rm = TRUE))

harvest_tonnes_usd_values <- harvest_tonnes_usd_geo_rgn %>%
  left_join(geo_rgn_values_df, by = c("georgn_id", "product")) %>%
  left_join(global_values_df, by = "product") %>%
  mutate(values = usd/tonnes) %>%
...
write.csv(harvest_tonnes_usd_values, "int/harvest_tonnes_usd_weighting_values.csv", row.names = FALSE)
```

Now prep the tonnes data from each product to be per each region for the maximum year (2017). 
```{r}
...
orn_tonnes <- np_tonnes %>%
  dplyr::filter(year == 2017, product == "ornamentals") %>%
...
#### NOTE: seaweed will be filled in with mariculture tonnes data when that is finished... use the commodities exports data as a place holder for now. 

sw_fill_df <- region_product %>%
  filter(product == "seaweeds")

sw_tonnes <- np_tonnes %>%
  dplyr::filter(year == 2017, product == "seaweeds") %>%
  full_join(sw_fill_df, by = c("rgn_id", "year", "product")) %>% ## full join to fill in the rest of the regions with 0
  mutate(tonnes = ifelse(is.na(tonnes), 0, tonnes),
         usd = ifelse(is.na(usd), 0, usd)) %>% ## gapfill the NAs to be 0
...
Now we will calculate the weights per each product. To do this we need to multiply our average $ value for each product * tonnes of each product, and then divide by the total per each region. 

```{r}
harvest_tonnes_usd_values <- read_csv("int/harvest_tonnes_usd_weighting_values.csv") %>%
  dplyr::select(rgn_id, product, year, value_per_tonne = values_final)

prod_weights <- orn_tonnes %>%
  bind_rows(sw_tonnes) %>%
  bind_rows(fofm_tonnes) %>% 
  left_join(harvest_tonnes_usd_values, by = c("rgn_id", "year", "product")) %>%
  arrange(rgn_id) %>%
...
old_prod_weights <- read_csv("../output/np_harvest_weights_from_usd.csv")

check <- prod_weights %>%
  rename("new_weight" = "weight") %>%
  left_join(old_prod_weights, by = c("rgn_id", "year", "product"))
