...
read.csv(here("globalprep/np/v2020/output/np_harvest_weights_from_usd.csv")) %>%
  filter(rgn_id==110)

read.csv(here("globalprep/np/v2019/output/np_harvest_weights_from_usd.csv")) %>%
  filter(rgn_id==110)

old <- read.csv(here("globalprep/np/v2019/output/np_harvest_tonnes.csv")) %>%
  filter(rgn_id==123) %>%
  rename(old_tonnes=tonnes)

tmp <- read.csv(here("globalprep/np/v2020/output/np_harvest_tonnes.csv")) %>%
  filter(rgn_id==123) %>%
  full_join(old) %>%
...
left_join(new_bahamas, by = c("commodity","year","rgn_id","rgn_name","product"))

plot(bahamas_compare$tonnes_new.x, bahamas_compare$tonnes)
abline(0,1, col="red")
