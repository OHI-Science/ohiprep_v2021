...
# Summary
This analysis converts FAO commodities data into data layers used to calculate OHI 2020 global natural products scores.  

# Updates from previous assessment
New year of FAO data (1976-2017), but no changes to general methods. Based on v2018 instead of v2019 due to gapfilling issues in v2019.
...
----------- | ------------ | ----------------- | -------------
196         |    Nigeria   |        -99        | No change in raw FAO data for any commodities from 2016 to 2017.  For all commodities, raw FAO data has NAs for both usd and tonnes in 2016 and 2017. After gapfilling, all commodities had values of 0 for both usd and tonnes in 2016 and 2017. In np_harvest_tonnes.csv, ornamentals had values of 0 for tonnes in 2016 and 2017, but seaweeds had a value of 28.5 tonnes in 2016, and 0 in 2017. In np_harvest_tonnes_rel.csv, relative tonnes stayed at 0 for ornamentals in 2016 and 2017, but for seaweeds, relative tonnes changed from 1 to 0 from 2016 to 2017. This could be the reason for the huge change in status. 
50          | Saudi Arabia |         0         | The following commodities had changes between 2016 and 2017 for raw FAO data: fish body oils, nei (NA -> 54 usd, NA -> 3 tonnes); fish liver oils, nei (1 -> NA usd, 7 -> NA tonnes); and seaweeds and other algae, unfit for human consumption (13 -> NA usd, 21 -> NA tonnes). After gapfilling, all commodities had values of 0 for both usd and tonnes in 2016 and 2017 except for the fish oils category (1 -> 55 usd, 7 -> 10 tonnes) from 2016 to 2017. In np_harvest_tonnes.csv, fish oil had a value of 1.75 tonnes in 2016, and 4.25 tonnes in 2017. Ornamentals, seaweeds, and sponges all had values of 0 for tonnes in 2016 and 2017. In np_harvest_tonnes_rel.csv, for fish oil, relative tonnes changed from 0.01 to 0.03 from 2016 to 2017. While this is a small change, it seems as if the change in fish oil quantity between the two years should've prompted some change in score?
75          |   Ukraine    |       +45         | The following commodities had changes between 2016 and 2017 for raw FAO data: agar agar nei (118 -> 47 usd, 7 -> 3 tonnes); and seaweeds and other algae, unfit for human consumption (38 -> 75 usd, 15 -> 16 tonnes). After gapfilling, all commodities had values of 0 for both usd and tonnes in 2016 and 2017 except for the seaweeds category (156 -> 122 usd, 22 -> 19 tonnes). The gapfilled values match up with the raw data in this case. In np_harvest_tonnes.csv, fish oil had a value of 1.75 tonnes in 2016, and 0 tonnes in 2017. Seaweeds had a value of 11.3 tonnes in 2016, and 13.7 tonnes in 2017. Shells and sponges had values of 0 for tonnes in 2016 and 2017. In np_harvest_tonnes_rel.csv, for fish oil, relative tonnes changed from 0.02 to 0 from 2016 to 2017. For seaweeds, relative tonnes were 1 for both 2016 and 2017, although there were slight differences in the actual tonnes value, as seen in the raw and gapfilled data. This doesn't seem like it should cause such a big positive change in status.



Comparing against last year's data. For example, compare the data for 1976-2015 for the 2019 and 2020 assessment year. Do not need to look at new years of data (2016-2017), which is only available in the 2020 assessment year, since those did not exist in the previous year's assessment.
...
old <- read.csv("../v2019/int/tonnes.csv") %>%  # CHANGE YEAR
  filter(rgn_id==68, year %in% c(1990:2015)) %>% 
...
old_tonnes <- read.csv("../v2019/output/np_harvest_tonnes.csv") %>%  # CHANGE YEAR
  filter(rgn_id==6, year %in% c(2010:2013)) %>% 
  rename(tonnes_old = tonnes)
...
## Take a look at Tunisia harvest tonnes: big increase in fish oil tonnes in 2013 from old assessment year to new assessment year
new <- read.csv("output/np_harvest_tonnes.csv") %>% 
  filter(rgn_id==61, year %in% c(2010:2013))
old <- read.csv("../v2019/output/np_harvest_tonnes.csv") %>%  # CHANGE YEAR
  filter(rgn_id==61, year %in% c(2010:2013)) %>% 
  rename(tonnes_old = tonnes)
...
smooth_old <- read.csv("../v2019/int/np_harvest_smoothed_data.csv") %>%  # CHANGE YEAR
  filter(rgn_id==61, year %in% c(2010:2013)) %>%
  select(rgn_id, product, year, tonnes, tonnes_rel) %>% 
...
## Look at Cuba final tonnes output: difference in final output for 2011-2013
new_tonnes <- read.csv("output/np_harvest_tonnes.csv") %>% 
  filter(rgn_id==112, year %in% c(2007:2013))
old_tonnes <- read.csv("../v2019/output/np_harvest_tonnes.csv") %>% # CHANGE YEAR
  filter(rgn_id==112, year %in% c(2007:2013)) %>% 
...
old <- read.csv("../v2019/int/usd.csv") # CHANGE YEAR
