...
Look at the wastewater project tif files to determine if they had the same problem (it looks like they did... so maybe it is ok?)

```{r}
fils <- list.files("/home/tuholske/plumes/output/", pattern="tif$", full.names = FALSE, recursive = TRUE)

files <- data_frame(fils) ## 134846
...
## Wastewater N effluent tif
wastewater_tif <- raster::raster(file.path("/home/shares/ohi/git-annex/land-based/wastewater/data/processed/N_effluent_output/global_effluent_2015_tot_N.tif"))
wasterwater_tif_log <- raster::calc(wastewater_tif, fun = function(x){log(x + 1)})
mapview(wasterwater_tif_log)

## they look similar... so far so good. 
...
sum(ws_pp_zonal_all_fix$effluent) #2343514672
12621550/2343514672 ## 0.00538

## should those be the same? 


## check wastewater project values
n_effluent_shp <- st_read(file.path("/home/tuholske/wastewater/data/processed/N_effluent_output/effluent_N_pourpoints_plumes_septic_N.shp"))
mapview(n_effluent_shp)
sum(n_effluent_shp$effluent) #316040362908

wastewater_effluent_values <- extract(wastewater_tif, n_effluent_shp, method = "simple", df = TRUE)
sum(wastewater_effluent_values$global_effluent_2015_tot_N) #22692476440
22692476440/316040362908 #0.0718
