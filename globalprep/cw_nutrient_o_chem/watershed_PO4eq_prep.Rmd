---
title: 'OHI 2020 - Land based nutrient and chemical '
...
[REFERENCE RMD FILE](http://ohi-science.org/ohiprep_v2020/globalprep/cw_nutrient_o_chem/v2020/watershed_PO4eq_prep.html)


# Summary
This document describes the steps for obtaining prepping data used for the land based nutrient and chemical data layer for clean waters. 
...
This is just code to visualize the new watershed PO4eq data, since I couldn't plot it effectively otherwise. 
```{r, eval = FALSE}
...
# test <- raster::raster(file.path(cw_nutrient_int_dir, "rasters/watersheds_zs_logged_raster.tif"))
# plot(test)
# crs(test)
```
...
st_write(ws_pp_zonal_all_fix, file.path(cw_nutrient_int_dir, "pourpoints/PO4eq_pourpoints.shp"), overwrite=TRUE, delete_dsn = TRUE)
...
```{r}
## look at ocean rasters: 

ocean <- raster::raster(file.path("~/grass_test/ocean.tif"))
plot(ocean)
crs(ocean)


## Lets use ocean mask, since it has same CRS as our shapefiles
ocean_mask <- raster::raster(file.path("~/grass_test/ocean_mask_landnull.tif"))
plot(ocean_mask)
crs(ocean_mask)

pp_test <- st_read(file.path("/home/shares/ohi/git-annex/land-based/wastewater/data/interim/pourpointstest/test5000.shp"))
```
