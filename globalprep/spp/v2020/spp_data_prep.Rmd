---
title: "OHI 2020 - Species subgoal (Biodiversity goal and resilience)"
...
* __Reference__: 
    * IUCN 2020. The IUCN Red List of Threatened Species. Version 2020-1. <http://www.iucnredlist.org>.
        * Shapefiles available from: https://www.iucnredlist.org/resources/spatial-data-download
        * __Downloaded__: April 1, 2020
    *BirdLife International and Handbook of the Birds of the World (2019) Bird species distribution maps of the world. Version 2019.1. BirdLife International, Cambridge, UK and NatureServe, Arlington, USA. http://datazone.birdlife.org/species/requestdis.
        * Zipped shapefile available from BirdLife International.  
        * __Downloaded__: May 6, 2020
...
* __Reference__:  
    *IUCN and UNEP-WCMC (2020), The World Database on Protected Areas (WDPA) [On-line], April 2020. Cambridge, UK: UNEP-WCMC. 
    * Zipped shapefile available at: www.protectedplanet.net.
    * __Downloaded__: April 24, 2020

* __Description__:   Shapefile of World Database on Protected Areas

* __Time range__: 1819 - 2020; some protected areas do not have an associated "status year" and are reported as year 0.

* __Format__:  Shapefile

*__File location__: `Mazu:git-annex/globalprep/_raw_data/wdpa_mpa/d2020/WDPA_Apr2020-shapefile/`
...
#2020
n_comp <- raster::raster(here("globalprep/spp/v2020/_output/n_spp_risk_raster_comp.tif"))
...
#2020
n_all <- raster::raster(here("globalprep/spp/v2020/_output/n_spp_risk_raster_all.tif"))
log_n_all <- log(n_all)
...
mean_risk_comp <- raster::raster(here("globalprep/spp/v2020/_output/mean_risk_raster_comp.tif"))

n_comp <- raster::raster(here("globalprep/spp/v2020/_output/n_spp_risk_raster_comp.tif"))

regions_ohi <- raster::raster(here("globalprep/spp/v2020/_spatial/eez_rast.tif"))
...
trend_comp <- raster::raster(here("globalprep/spp/v2020/_output/trend_raster_comp.tif"))

n_trend_comp <- raster::raster(here("globalprep/spp/v2020/_output/n_trend_raster_comp.tif"))
...
write.csv(status, here("globalprep/spp/v2020/output/sp_status_global.csv"), row.names=FALSE)  
...
```{r, eval = FALSE}
status <- read_csv(here("globalprep/spp/v2020/output/sp_status_global.csv"))

old_spp <- read.csv(here("globalprep/spp/v2019/output/sp_status_global.csv")) %>%
...
status <- read.csv(here("globalprep/spp/v2020/output/sp_status_global.csv")) %>%
...
write.csv(trend, here("globalprep/spp/v2020/output/sp_trend_global.csv"), row.names=FALSE)  
...
trend <- read.csv(here("globalprep/spp/v2020/output/sp_trend_global.csv"))  

old_spp <- read.csv(here("globalprep/spp/v2019/output/sp_trend_global.csv")) %>%
  rename(old_score = score) %>%
  left_join(trend) 
...
write.csv(status, here("globalprep/spp/v2020/output/sp_status_global_gf.csv"), row.names=FALSE)          

trend <- read.csv(here("globalprep/spp/v2020/output/sp_trend_global.csv")) %>%
  mutate(gapfilled = ifelse(rgn_id == 232, 1, 0)) %>%
  mutate(method = ifelse(rgn_id == 232, "mean of neighbors", NA)) %>%
  dplyr::select(rgn_id, gapfilled, method)
write.csv(trend, here("globalprep/spp/v2020/output/sp_trend_global_gf.csv"), row.names=FALSE)          
...
# saved in Mazu:spp/v2020
projectRaster(risk_x_n, rgns, method="ngb", over=TRUE, 
              filename=file.path(dir_M, "git-annex/globalprep/spp/v2020/int/risk_x_n_comp_mol.tif"),
              progress="text")
projectRaster(n_comp, rgns, method="ngb", over=TRUE, 
              filename=file.path(dir_M, "git-annex/globalprep/spp/v2020/int/n_comp_mol.tif"),
              progress="text")
...
risk_x_n_mol <- raster(file.path(dir_M, "git-annex/globalprep/spp/v2020/int/risk_x_n_comp_mol.tif"))
n_mol <- raster(file.path(dir_M, "git-annex/globalprep/spp/v2020/int/n_comp_mol.tif"))
...
projectRaster(trend_x_n, rgns, method="ngb", over=TRUE, 
              filename=file.path(dir_M, "git-annex/globalprep/spp/v2020/int/trend_x_n_comp_mol.tif"),
              progress="text")
projectRaster(n_trend_comp, rgns, method="ngb", over=TRUE, 
              filename=file.path(dir_M, "git-annex/globalprep/spp/v2020/int/n_trend_comp_mol.tif"),
              progress="text")
...
trend_3nm_mol <- raster(file.path(dir_M, "git-annex/globalprep/spp/v2020/int/trend_x_n_comp_mol.tif"))
n_trend_3nm_mol <- raster(file.path(dir_M, "git-annex/globalprep/spp/v2020/int/n_trend_comp_mol.tif"))


trend_stack <- stack(trend_3nm_mol, n_trend_3nm_mol)

trend_df <- raster::zonal(trend_stack, rgns_3nm, fun='sum')
...
# get eez value:
eez_status <- read.csv(here("globalprep/spp/v2020/output/sp_status_global.csv")) %>%
  filter(rgn_id == 19) %>%
...
resilience <- res_gf %>%
  dplyr::select(rgn_id, year, score = spp_status)
dim(resilience)  # 220 * 9
summary(resilience) # should be no NA values

write.csv(resilience, here("globalprep/spp/v2020/output/sp_status_3nm.csv"), row.names=FALSE)  

```

### Resilience: save a record of gapfilling
```{r, eval = FALSE}
res <- read.csv(here("globalprep/spp/v2020/output/sp_status_3nm.csv")) %>%
  mutate(gapfilled = ifelse(rgn_id == 19, 1, 0)) %>%
  mutate(method = ifelse(rgn_id == 19, "eez scale data used", NA)) %>%
  dplyr::select(rgn_id, year, gapfilled, method)
write.csv(res, here("globalprep/spp/v2020/output/sp_status_3nm_gf.csv"), row.names=FALSE)          
...
status <- read.csv(here("globalprep/spp/v2020/output/sp_status_3nm.csv")) %>%
...
old_spp <- read.csv(here("globalprep/spp/v2019/output/sp_status_3nm.csv")) %>%
  filter(year == max(year)) %>%
  rename(old_score = score) %>%
  mutate(year = 2020) %>%
  left_join(status) %>%
