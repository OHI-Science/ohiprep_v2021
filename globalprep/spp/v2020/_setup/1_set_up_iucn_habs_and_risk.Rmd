...
if(!file.exists(spp_habs_from_api_file) | reload) {
  ...
  for(j in 1:n_chunks) { 
    ###j <- 16
    spp_index <- c( ((j - 1) * chunk_size + 1) : min((j * chunk_size), length(spp_ids_all)) )#[1:100]
...
### Always check how many observations there after a join. 2019: 13404, 2020: 16019
...
if(!file.exists(spp_timeseries_file) | reload) {
  ...
  cat_msg('elapsed time: ', (proc.time() - ptm)[3]) #2020 elapsed time:  1339.143 
